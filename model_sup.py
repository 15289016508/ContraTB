...
class DocumentModel(nn.Module):
    def __init__(self, config):
        ...
        dim_word = 128
        self.character_embedding = nn.Embedding(len(config.vocab_words), dim_word, padding_idx=0).to(self.device)
        self.positional_embedding = nn.Embedding(12450, dim_word, padding_idx=0).to(self.device)
        self.sentence_position_embedding = nn.Embedding(4009, 300, padding_idx=0).to(self.device)
        ...
        self.layer_norm_lstm_w = nn.LayerNorm(
            2 * config.hidden_size_lstm_sentence)  # Add LayerNorm after word-level LSTM
        self.layer_norm_lstm_d = nn.LayerNorm(
            2 * config.hidden_size_lstm_sentence)  # Add LayerNorm after document-level LSTM
    ...
    def forward(self, word_ids_tensor, position_idf_tensor, sentence_idf_tensor, document_lengths, sentence_length):
        ...
        character_embeddings = self.dropout(combined_data.float().to(self.device))
        ...
        packed_output, (h_n, c_n) = self.bilstm_w(packed_input)
        output, _ = pad_packed_sequence(packed_output, batch_first=True)
        U_sent = F.elu(torch.matmul(output, self.w_word) + self.b_word)
        ...
        # 将掩码应用于注意力分数
        # output1 = torch.matmul(U_sent, self.u_word).view(-1, s[2])
        ...
        # A = F.softmax(torch.matmul(U_sent, self.U_word).view(-1, s[2]), dim=-1)
        self.attention_w = A
        # print(A.shape)
        # print("A", A)
        ...
        sliced_outputs = torch.stack(padded_sentence_outputs)
        ...
        packed_document_representation, (h_n, c_n) = self.bilstm_d(sentence_representation)

        output, _ = pad_packed_sequence(packed_document_representation, batch_first=True)
        U_sent = F.elu(torch.matmul(output, self.w_s) + self.b_s)
        ...
        for i, length in enumerate(document_lengths):
            mask_tensor[i, length:] = True
        # 将掩码应用于注意力分数
        masked_attention_scores = (torch.matmul(U_sent, self.u_s).view(-1, s[1]).masked_fill(mask_tensor,
                                                                                             float("-inf")))
        # print("masked_attention_scores_word", masked_attention_scores)
        # 计算注意力权重
        A = F.softmax(masked_attention_scores, dim=-1)
        ...
        # print("lstm_output_sentence_attention", output.shape)
        dropout1 = self.dropout(output)
        dropout2 = self.dropout(output)
        # Concatenate the results along a new dimension
        document_representation = torch.stack([dropout1, dropout2], dim=1)
        # print("document_representation", document_representation.shape)
